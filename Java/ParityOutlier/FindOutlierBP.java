...
public class FindOutlierBP{
    public static int find(int[] integers) {
        ...
        int mod = (sum == 0 || sum == 1) ? 1 : 0; // we can remplace it by (sum < 2)

        return Arrays.stream(integers).parallel() // call parallel to get as much bang for the buck on a "large" array
                .filter(n -> Math.abs(n) % 2 == mod).findFirst().getAsInt(); // findAny() instead of findFirst() is a solution too
    }
}